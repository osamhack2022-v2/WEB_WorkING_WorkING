...
import { config } from "../../";
import {
  MeilingV1OAuthAccessTokenInfo,
  MeilingV1OAuthOpenIDData,
} from "./interface";
...
export async function getUser(
  accessToken: string
): Promise<MeilingV1OAuthOpenIDData | false> {
  try {
    const data = await axios.get(
      config.meiling.hostname + "/v1/oauth2/userinfo",
      {
        headers: {
...
export async function getToken(
  accessToken: string
): Promise<MeilingV1OAuthAccessTokenInfo | undefined> {
  try {
    const data = await axios.get(
      config.meiling.hostname + "/v1/oauth2/tokeninfo",
      {
        params: {
